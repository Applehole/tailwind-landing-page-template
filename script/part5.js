...
*/
const PART5_2_5 = `select title from content where content.id = (select roleId from user where name = 'jiSungPark')`;
/*
----------------------------------------------------------------------------------------------
    TODO: Q 5-2-6. JiSungPark이 작성한 content의 category name을 찾기위한 SQL을 작성해주세요.
*/

const PART5_2_6 = `select * from category where category.id in (select categoryId from content_category where content_category.contentId = (select id from content where content.userId = (select id from user where name = 'jiSungPark')))`;
/*
----------------------------------------------------------------------------------------------
    TODO: Q 5-2-7. category의 name이 soccer인 content의 title, body, created_at을 찾기위한 SQL을 작성해주세요.
*/
const PART5_2_7 = `select title,body,created_at from content where content.id in (select contentId from content_category where content_category.categoryId = (select id from category where name = 'soccer'))`;
/*
----------------------------------------------------------------------------------------------
    TODO: Q 5-2-8. category의 name이 soccer인 content의 title, body, created_at, user의 name을 찾기위한 SQL을 작성해주세요.
*/
const PART5_2_8 = `select distinct content.title, content.body, content.created_at, user.name from category 
...
*/
const PART5_2_9 = `select count(*) as ContentCount from content where content.userId = (select id from user where name = 'duRiCha')`;
/*
----------------------------------------------------------------------------------------------
    TODO: Q 5-2-10. 각 user(컬럼명: name)가 작성한 글의 개수 (컬럼명: ContentCount)를 출력하기 위한 SQL을 작성해주세요.
*/
const PART5_2_10 = `
select user.name, count(content.id) AS ContentCount From content
